    ...
    module Shareable #:nodoc:
      ...
      module SingletonMethods
        ...
        def find_shares_by_user(user, *opts)
          shareable = ActiveRecord::Base.send(:class_of_active_record_descendant, self).to_s
          options = {
        ...
        def find_by_shared_to(object, *opts)
          shareable = ActiveRecord::Base.send(:class_of_active_record_descendant, self).to_s
          shared_to = ActiveRecord::Base.send(:class_of_active_record_descendant, object.class).to_s
          options = {
            :joins=>"LEFT OUTER JOIN shares s ON s.shareable_id = #{self.table_name}.id",
            :select=>"#{self.table_name}.*",
      ...
      module InstanceMethods
        ...
        def remove_share_from(object, by_user)
          shareable = ActiveRecord::Base.send(:class_of_active_record_descendant, self.class).to_s
          to = ActiveRecord::Base.send(:class_of_active_record_descendant, object.class).to_s
        ...
        def find_shared_to_by_type(shared_type, *opts)
          shareable = ActiveRecord::Base.send(:class_of_active_record_descendant, self.class).to_s
          shared_to = ActiveRecord::Base.send(:class_of_active_record_descendant, shared_type)
          options = {
